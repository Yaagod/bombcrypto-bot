...
from src.windowUtils import activate_linux_window, get_linux_bombcrypto_windows, get_linux_window_name
...
def main():
    ...
    # window based time intervals dictionary
    last = {}
    # =========

    while True:
        bomb_windows = get_linux_bombcrypto_windows()

        for window in bomb_windows:

            window_name = get_linux_window_name(window)
            # ignore all bot related windows, like the code editor and the terminal
            if 'bot' in window_name:
                continue

            # add the window id to time intervals
            if window not in last:
                last[window] = {"login": 0,
                                "heroes": 0,
                                "new_map": 0,
                                "check_for_captcha": 0,
                ...
                logger('⚒️ New Bombcrypto window found')

            logger(f'⚒️ Changing to {window_name}')
            activate_linux_window(window)
            time.sleep(2)

            now = time.time()
            if now - last[window]["check_for_captcha"] > addRandomness(t['check_for_captcha'] * 60):
                last[window]["check_for_captcha"] = now

            if now - last[window]["heroes"] > addRandomness(t['send_heroes_for_work'] * 60):
                last[window]["heroes"] = now
                refreshHeroes()

            if now - last[window]["login"] > addRandomness(t['check_for_login'] * 60):
                sys.stdout.flush()
                last[window]["login"] = now
                login()

            if now - last[window]["new_map"] > t['check_for_new_map_button']:
                last[window]["new_map"] = now

                if clickBtn(images['new-map']):
                    loggerMapClicked()

            if now - last[window]["refresh_heroes"] > addRandomness(t['refresh_heroes_positions'] * 60):
                last[window]["refresh_heroes"] = now
